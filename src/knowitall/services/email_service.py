...
def _build_digest_html(digest) -> str:
    ...
    parts: list[str] = [
        ...
        f"<h1 style='color:#58a6ff'>🧠 {escape(digest.headline)}</h1>",
    ...
    if digest.papers:
        ...
        for p in digest.papers:
            link = escape(p.project_url or p.pdf_url or "", quote=True)
            parts.append(
                f"<li><a style='color:#58a6ff' href='{link}'>{escape(p.title)}</a>"
    ...
    if digest.startup_ideas:
        ...
        for idea in digest.startup_ideas:
            parts.append(f"<li><b>{escape(idea.title)}</b>: {escape(idea.core_insight[:200])}</li>")
        parts.append("</ul>")

    if digest.tools:
        parts.append("<h2 style='color:#58a6ff'>🛠 Tech & Tools</h2><ul>")
        for t in digest.tools:
            parts.append(f"<li><b>{escape(t.name)}</b> — {escape(t.hype_level.value)}</li>")
        parts.append("</ul>")

    if digest.opportunities:
        parts.append("<h2 style='color:#58a6ff'>🎯 Opportunities</h2><ul>")
        for o in digest.opportunities:
            parts.append(f"<li><b>{escape(o.title)}</b> — ROI {o.roi_score:.2f}</li>")
        parts.append("</ul>")
...
def _build_scout_html(report) -> str:
    ...
    parts: list[str] = [
        "<html><body style='font-family:sans-serif;background:#0d1117;color:#e6edf3;padding:20px'>",
        f"<h1 style='color:#58a6ff'>🔍 {escape(report.headline or 'Scout Report')}</h1>",
    ...
    if report.tech_trends:
        ...
        for t in report.tech_trends:
            parts.append(f"<li><b>{escape(t.title)}</b> — interest {t.interest_score:.2f}</li>")
        parts.append("</ul>")

    if report.complaints:
        parts.append("<h2 style='color:#58a6ff'>😤 Complaints</h2><ul>")
        for c in report.complaints:
            parts.append(f"<li>{escape(c.summary)} — severity {c.severity:.2f}</li>")
        parts.append("</ul>")

    if report.industry_gaps:
        parts.append("<h2 style='color:#58a6ff'>🕳️ Industry Gaps</h2><ul>")
        for g in report.industry_gaps:
            parts.append(f"<li><b>{escape(g.title)}</b> — opportunity {g.opportunity_score:.2f}</li>")
        parts.append("</ul>")

    if report.startup_ideas:
        parts.append("<h2 style='color:#58a6ff'>🚀 Startup Ideas</h2><ul>")
        for i in report.startup_ideas:
            parts.append(f"<li><b>{escape(i.title)}</b>: {escape(i.description[:200])}</li>")
        parts.append("</ul>")
